...
try:
    ...
    db = server.create('misteryland_k')
except:
    #Caso contrario solo conectarse a la base existente
    db = server['misteryland_k']
    
#Aqui se define el bounding box con los limites geograficos donde recolectar los tweets
twitterStream.filter(locations=[-80.3111,-2.3738,-79.5427,-1.8014])
twitterStream.filter(track=['electronica', 'musica', 'dj', 'baile', 
